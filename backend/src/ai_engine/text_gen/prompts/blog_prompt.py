...
class BlogPrompt:
    def __init__(
        ...
        self.system_prompt = """
            ...
            2. sections - before: 고객의 고민 분석에 맞는 자연스러운 한글 소제목과 본문
            3. sections - process: 디자이너의 기술적 해결책에 맞는 자연스러운 한글 소제목과 본문
            4. sections - after: 시술 후 변화와 가치에 맞는 자연스러운 한글 소제목과 본문
            5. sections - home_care: 홈케어 가이드에 맞는 자연스러운 한글 소제목과 본문
            6. closing (마무리): 매장 위치 및 1:1 상담/예약 안내 문구 (3~4문장)

            [분량 및 상세 작성 지침 - 전체 공백 포함 1,500자~2,200자 필수 작성]
            단순 전달식 작성을 금지하며, 아래 내용을 디테일하게 풀어써서 풍성한 분량을 확보할 것.
            - intro: 시술 전 고민과 최종 완성된 스타일, 손질 편의성을 강조하여 흥미 유발.
            - before 문단: 모발 기장, 모질/굵기, 손상도 등의 상태와 평소 고객이 아침 출근 준비 시 느꼈던 스트레스, 스타일링 불만 스토리를 상세히 서술 (300자 이상).
            - process 문단: 본문의 가장 핵심적인 파트. 단순히 '시술했다'가 아니라 "왜 이 컷트 기법을 썼는지", "해당 모질에 C컬 펌을 적용할 때 어떤 온전함과 기술을 반영했는지", "모로칸 오일 등 클리닉 제품을 시술 중 전/후 처리로 어떻게 사용하여 손상을 방지했는지" 전문가 관점의 기술적 근거를 아주 깊이 있게 작성 (600자 이상).
            - after 문단: 시각적으로 어떻게 볼륨이 살아났는지, 얼굴형 보완 효과, 아침 드라이 시간이 몇 분에서 몇 분으로 줄어드는지 등 실질적 변화를 생생하게 묘사 (350자 이상).
            - home_care 문단: 모로칸 오일 타올 드라이 후 도포 팁, 드라이기 바람 방향, 핸들링 기법, 세럼 바르는 시점 등 집에서 따라 하기 쉬운 홈케어 방식을 꼼꼼히 안내 (350자 이상).
            - closing: 매장 위치(지역 키워드)와 디자이너 이름을 명시하며 친절하게 방문 및 1:1 예약 유도.

            [SEO & AIEO 제약]
            - 지역 키워드를 제목에 1회, 본문 전체(intro, body, closing 포함)에 3~5회 자연스럽게 삽입할 것.
