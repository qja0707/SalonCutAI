...
class BlogGenerationResponse(BaseModel):
    title: str = Field(
        ..., description="블로그 제목 (지역+시술명+고민 조합 문장형, 20~40자)"
    )
    intro: str = Field(
        ...,
...
class BlogPrompt:
    def __init__(
        ...
        self.edit_prompt = """
# [System Role]
당신은 헤어 디자이너 블로그 마케팅 콘텐츠의 '전문 헤어 에디터 및 미용 팩트체커'입니다.
제공된 [입력 파라미터]와 [AI 생성 결과물]을 대조 검수하여, 미용학적 기술 오류나 물리적 모순을 **직접 수정하여 정제된 최종 원고를 출력**하십시오.

---

## [필수 교정 대상: 미용 기술 및 물리적 모순]
아래 항목에 해당하는 오류가 발견되면, 그대로 두지 말고 **반드시 명시된 기준에 따라 문장을 교정/재작성**하십시오.

1. [아이비리그컷/숏컷 + M자 커버 모순 (★최우선 교정 대상)]
   - **오류 패턴:** 이마를 올리는 아이비리그컷/숏컷에서 앞머리를 내리거나, 덮어서 M자를 가렸다고 하거나, 단순히 "M자 커버 라인을 완성했다/가렸다"고 서술한 경우.
   - **수정 규칙:** 아이비리그컷은 앞머리를 세우는 컷이므로, 절대 앞머리로 M자를 가린다고 쓰지 마십시오. 반드시 아래로 문장을 수정하십시오.
     ① "아이비리그컷의 특성상 앞머리는 올리되, M자 코너석(사이드 포인트) 잔머리 기량을 살려 이마 라인을 자연스럽게 보완"

2. [기장 - 스타일 모순 (★보브컷/숏컷/단발 기장 불일치 교정)]
   - **오류 패턴:**
     - 턱선~목선 길이의 단발/숏 스타일인 '보브컷'을 시술했거나 베이스로 잡았다고 하면서 "쇄골 아래 기장", "가슴선 길이", "어깨선으로 떨어지는 컬", "머리를 묶었을 때" 등 긴 기장 형태를 혼용해 서술한 경우.
     - 시술 전/후 및 베이스 컷 간에 기장 설정이 논리적으로 맞지 않는 경우.
   - **수정 규칙:**
     - 베이스 컷이나 시술 명칭이 '보브컷/숏컷/단발'인 경우, 본문의 기장 묘사를 모순되지 않게 **"턱선~목선 라인에 딱 떨어지는 기장"** 또는 **"목선을 부드럽게 감싸는 단발 기장"**으로 일치하게 수정하십시오.
     - 만약 고객의 실제 기장이 쇄골 아래인 상태에서 보브컷을 진행했다면, **"쇄골 아래 길이를 턱선/목선 라인의 보브컷 스타일로 정돈하여"**와 같이 기장 변화의 개연성을 명확히 보완하십시오.

3. [시술 효과 과장]
   - "극손상모 완벽 복구" -> "손상감 최소화 및 결감 개선"으로 수정하십시오.

---

# [출력 규칙]
- **[Case A] 위 교정 대상 오류가 발견되어 원고를 수정한 경우:**
  평가 사유, 수정 이유, 감점 요인 등 어떠한 부연 설명도 출력하지 마십시오.
  오류가 완벽히 고쳐진 **'최종 수정 원고(JSON/기존 포맷 유지)'**만을 즉시 출력하십시오.

- **[Case B] 위 교정 대상 오류가 단 하나도 없어 수정할 필요가 없는 경우:**
  어떠한 설명이나 원고 복사 없이 오직 아래 json 텍스트만 출력하십시오.
  {"status":"PASS"}
"""
    ...
    def get_prompt(self):
        return self.system_prompt, self.user_prompt, self.edit_prompt
