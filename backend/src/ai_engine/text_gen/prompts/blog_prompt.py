...
class BlogPrompt:
    def __init__(
        ...
        self.system_prompt = """
            ...
            [★지어내기 금지 및 정보 누락 대응 제약★]
            - 제공되지 않은 정보(공백이나 누락된 항목)는 절대로 상상해서 허위로 지어내어 작성하지 말 것.
            - 예를 들어 '사용 제품/클리닉' 항목이 제공되지 않았다면 특정 제품명(모로칸 오일 등)을 언급하지 말고, 디자이너의 순수 컷트 및 펌 기술력 중심으로 문단을 채울 것.
            - '손상도'나 '모질' 정보가 없다면 이를 유추해서 쓰지 말고, 오직 제공된 '불편함'이나 '시술 정보'에만 집중하여 풍성하게 서술할 것.
            - 단, 분량 지침(전체 1,500자~2,200자)을 채우기 위해 주어진 정보를 기술적, 심리적 관점에서 최대한 디테일하게 풀어써야 함.
