...
async function createBlogJob(scenario) {
  const response = await fetch(`${base}/api/v1/text-gen/blog-generation`, {
    method: "POST",
    headers: {
  ...
  assert(
    response.status === 202,
    `${scenario} 블로그 생성 응답: ${response.status}`,
  );
  const body = await response.json();
  assertExactKeys(
    body,
    ["title", "intro", "sections", "closing", "hashtags"],
    "블로그 접수 외피",
  );
  return body;
}
...
async function verifyReferenceFaces() {
  ...
  assert(
    thumbnail.headers.get("content-type")?.startsWith("image/svg+xml"),
    "참조 얼굴 썸네일 Content-Type",
  );
  assert(
    (await thumbnail.arrayBuffer()).byteLength > 100,
    "참조 얼굴 썸네일 바이트",
  );

  const missing = await fetch(
    `${base}/api/v1/reference-faces/ref-unknown/thumbnail`,
  );
  assert(
    missing.status === 404,
    `없는 참조 얼굴 썸네일 응답: ${missing.status}`,
  );
...
async function verifyFaceOptionValidation() {
  ...
  const cases = [
    ...
    [
      "양쪽 동시 지정",
      withFace({ ...referenceFace, prompt: promptFace.prompt }),
    ],
    [
      "없는 참조 얼굴",
      withFace({
        ...referenceFace,
        reference: { reference_face_id: "ref-unknown" },
      }),
    ],
    [
      "목록에 없는 국적",
      withFace({
        ...promptFace,
        prompt: { ...promptFace.prompt, ethnicity: "화성인" },
      }),
    ],
    [
      "필수 연령대 누락",
      withFace({ ...promptFace, prompt: { ...promptFace.prompt, age: "" } }),
    ],
    ...
    [
      ...
      withFace({
        ...promptFace,
        prompt: { ...promptFace.prompt, makeup: "스모키" },
      }),
    ],
    [
      "옛 연령대 값",
      withFace({
        ...promptFace,
        prompt: { ...promptFace.prompt, age: "20대 초반" },
      }),
    ],
  ];
...
async function verify() {
  ...
  let response = await fetch(`${base}/api/v1/text-gen/blog-generation`, {
    method: "POST",
  ...
  assert(
    invalidBlog.error?.code === "INVALID_BLOG_INPUT",
    "블로그 입력 오류 코드",
  );
  assert(
    invalidBlog.error?.retryable === false && invalidBlog.request_id,
    "블로그 입력 오류 외피",
  );
  ...
  const [faceEntries] = await Promise.all([
    Promise.all(
      scenarios.map(async (scenario) => [
        scenario,
        await createFaceSwapJob(scenario),
      ]),
    ),
    Promise.all(
      ["normal", "blog-fail"].map(async (scenario) => [
        scenario,
        await createBlogJob(scenario),
      ]),
    ),
  ]);
    ...
    Promise.all(
      scenarios.map(async (scenario) => {
        const faceResponse = await fetch(
          `${base}/api/v1/face-swap-jobs/${created[scenario].job_id}`,
        );
        assert(
          faceResponse.ok,
          `${scenario} 조회 응답: ${faceResponse.status}`,
        );
  ...
  const expected = {
    normal: "completed",
    "image-fail": "failed",
    "face-not-detected": "failed",
  };
  for (const scenario of scenarios) {
    assert(
      jobs[scenario].status === expected[scenario],
      `${scenario}: ${jobs[scenario].status}`,
    );
  }
  assert(
    typeof jobs.normal.consent_recorded_at === "string",
    "정상 동의 기록 시각",
  );
  assert(
    !Number.isNaN(Date.parse(jobs.normal.consent_recorded_at)),
    "정상 동의 기록 시각 형식",
  );
  assert(
    jobs["image-fail"].error?.code === "IMAGE_GENERATION_FAILED",
    "재시도 가능 이미지 오류 코드",
  );
  assert(
    jobs["image-fail"].error?.retryable === true,
    "재시도 가능 이미지 retryable",
  );
  assert(
    jobs["face-not-detected"].error?.code === "FACE_NOT_DETECTED",
    "얼굴 미검출 오류 코드",
  );
  assert(
    jobs["face-not-detected"].error?.retryable === false,
    "얼굴 미검출 retryable",
  );
  ...
  for (const ratio of ["1x1", "4x5", "9x16"]) {
    const imageResponse = await fetch(
      `${base}/api/v1/face-swap-jobs/${normalId}/images/${ratio}`,
    );
    ...
    assert(
      imageResponse.headers.get("content-disposition")?.includes("filename="),
      `${ratio} Content-Disposition`,
    );
    assert(
    ...
    assert(
      (await imageResponse.arrayBuffer()).byteLength > 100,
      `${ratio} 이미지 바이트`,
    );
  }
  ...
  const [imageRetryResponse] = await Promise.all([
    fetch(`${base}/api/v1/face-swap-jobs/${retryableId}/retry`, {
      method: "POST",
    }),
  ]);
  assert(
    imageRetryResponse.status === 202,
    `이미지 재시도 응답: ${imageRetryResponse.status}`,
  );
  ...
  assert(
    retried.status === "completed" && retried.attempt === 2,
    "이미지 재시도 완료/attempt",
  );

  const nonRetryableId = created["face-not-detected"].job_id;
  response = await fetch(
    `${base}/api/v1/face-swap-jobs/${nonRetryableId}/retry`,
    { method: "POST" },
  );
  assert(
    response.status === 409,
    `재시도 불가 이미지 응답: ${response.status}`,
  );
  ...
  const faceForm = readFileSync(
    resolve("src/components/face-option-form.tsx"),
    "utf8",
  );
  assert(
    faceUi.includes("buildFaceOption(face)"),
    "얼굴 옵션을 payload 로 전송",
  );
  ...
  const faceRoute = readFileSync(
    resolve("src/app/api/v1/face-swap-jobs/route.ts"),
    "utf8",
  );
  assert(
    faceRoute.includes("FACE_REQUIRED_ALLOWED"),
    "필수 얼굴 옵션 목록 대조",
  );
  assert(
    faceRoute.includes("FACE_OPTIONAL_ALLOWED"),
    "세부 얼굴 옵션 목록 대조",
  );
  // 배경 교체는 기능 2로 넘어갔다. 토글이 되살아나면 지원하지 않는 모드가 나간다.
  assert(
    faceUi.includes("BACKGROUND_REPLACE_READY = false"),
    "배경 교체 토글 비활성",
  );

  const blogUi = readFileSync(
    resolve("src/app/generate/blog/blog-generator.tsx"),
    "utf8",
  );
  const blogCopy = readFileSync(
    resolve("src/lib/api-client/blog-content.ts"),
    "utf8",
  );
  const apiClient = readFileSync(
    resolve("src/lib/api-client/client.ts"),
    "utf8",
  );
  const legacyRoute = readFileSync(
    resolve("src/app/api/generate-blog/route.ts"),
    "utf8",
  );
  const blogFields = readFileSync(
    resolve("src/app/generate/blog/blog-fields.tsx"),
    "utf8",
  );
  assert(
    blogUi.includes('searchParams.get("label")'),
    "블로그 연결 컨텍스트 배지 유지",
  );
  assert(
    blogUi.includes("buildBlogPayload(fields, profile)"),
    "12필드 payload 로 전송",
  );
  assert(
    blogUi.includes("isBlogFieldsReady(fields)"),
    "필수 4개 미입력 시 제출 차단",
  );
  assert(
    !blogUi.includes("BLOG_TONES"),
    "톤 앤 매너 입력 제거 (12필드에 없음)",
  );
  assert(
    blogFields.includes("formatDuration"),
    "소요 시간 표시값과 전송값 분리",
  );
  assert(
    blogFields.includes("buildRegionKeyword"),
    "region_keyword 지역+업종 조합",
  );
  assert(
    !blogUi.includes('provider: "openai"'),
    "클라이언트 provider 고정값 제거",
  );
  assert(
    [
      "result.title",
      ...
      "result.closing",
      "result.hashtags",
    ].every((value) => blogCopy.includes(value)),
    "복사 문자열 결과 구조 일치",
  );
