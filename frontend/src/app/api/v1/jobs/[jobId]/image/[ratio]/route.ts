...
import { getMockJob, mockImage } from "@/lib/api-client/server/mock-store";
...
export async function GET(_request: Request, context: Context) {
  ...
  if (job.image.status !== "completed") {
    return errorResponse(409, "NOT_RETRYABLE", "이미지 생성이 아직 완료되지 않았습니다.");
  }
  const image = mockImage(jobId, ratio);
  if (!image) return errorResponse(409, "NOT_RETRYABLE", "요청한 규격의 결과를 찾을 수 없습니다.");
