...
**세 기능 화면은 공통 구조를 씁니다** (#156·#157·#158·#159). 헤더는 `components/flow/page-shell.tsx`
하나로 모았고, 단계 흐름은 얼굴 교체 화면에만 있던 것을 `components/flow/step-flow.tsx` 로
일반화해 블로그·숏츠에도 적용했습니다. 폰에서는 한 번에 한 단계만 보이고(#123), 하단 고정 바가
진행 상태와 다음 버튼을 함께 들고 있습니다. 화면을 새로 만들 때는 이 둘을 먼저 보세요.

**MVP 3기능 (Discussion #23 기준)**

| 라우트 | 메뉴 이름 / 화면 대제목 | 상태 |
|---|---|---|
| `/face-swap` | AI 모델로 얼굴 변경 / **헤어 모델 만들기** | job 기반 mock 완성 (접수→폴링→재시도→3규격→삭제). 동의 확인(#173 — 토글·체크박스를 버튼 하나로), 얼굴 옵션 7축(#77), AI 생성 배지·규격 쓰임새(#75), 결과 흐름 개편(#105 — 겹쳐 가르는 슬라이더·대기 3단계 안내·같은 설정으로 다음 사진), 복구·교체 시 사진 짝 잠금(#109), 그 자리에서 다시 생성(#185·#204). **AI 모델 고르기는 2열 격자에 닉네임**(#204) — 참조 얼굴 53장(#201 로 21장 추가, 신규분이 목록 앞). **공개 경로에서 실제 연동 검증 완료** |
...
| `/generate/shorts` | AI 숏츠 만들기 / **간편 숏츠 만들기** | **backend 연결 완료** — `/api/v1/video-jobs`(#68)로 접수→폴링→9:16 재생·다운로드까지 동작합니다. 클립 2~8개·역할(시술 전/과정/디테일/마무리)·사용 구간·자막(AI 자막 생성 포함)·얼굴 블러(기본 켜짐, 끌 수 있음)·24시간 TTL. 폰 동선 다듬기(#110), 세로 3단계 화면으로 재구성(#195), 업로드 전 로그인 확인(#193) |

**공통**

| 라우트·파일 | 내용 |
|---|---|
| `/` | 랜딩 홈 (#111·#112) — 혜택 중심 카피, 실제 결과물 히어로, Before/After 슬라이더. 히어로 통합·01·02·03 섹션 실측 정리(#165), 03 섹션 실제 시술 영상(#176), 예시 체험 버튼이 예시 사진을 실은 채 열림(#121) |
...
## backend 계약 (2026-08-27 기준 · dev `fce032a`)
...
- **참조 얼굴**: `/api/v1/reference-faces` 가 정본이고 **응답 순서가 곧 화면 순서**입니다.
  8/27 기준 53장(#201 로 ref-33~53 21장 추가, backend 가 목록 앞에 배치). label 은
  `"한국인 20대 여성 A"` 형태이고, 화면에 보이는 닉네임은 프론트 `src/lib/face-nicknames.ts`
  가 id 로 얹습니다 — 매핑에 없는 id 는 label 로 되돌아가므로 얼굴이 더 들어와도 칸이 비지
  않습니다. mock(`server/mock-store.ts`)은 **개수와 순서까지** 실서버와 맞춥니다
...
- **영상(숏츠)**: `/api/v1/video-jobs`(#68). `multipart/form-data`로 클립 2~8개와 옵션(역할·사용 구간·자막)을 올리고 job_id 로 폴링합니다. **얼굴 블러는 기본 켜짐이고 화면 스위치로 끌 수 있습니다**(#180). 결과는 9:16, 24시간 TTL. job 저장이 서버 프로세스 메모리라 **서버가 재시작되면 진행 중이던 작업은 사라집니다**
